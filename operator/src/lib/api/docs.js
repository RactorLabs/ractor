...
export function getApiDocs(base) {
  ...
  },
  {
    ...
    endpoints: [
      { method: 'GET', path: '/api/v0/agents', auth: 'bearer', desc: 'List/search agents with pagination.', params: [
        ...
        { in: 'query', name: 'tags', type: 'string (comma-separated)', required: false, desc: 'Filter by tags (UNION/OR). Provide multiple tags as a comma-separated list (e.g., tags=prod,team). Tags are matched case-insensitively and stored lowercase.' },
  ...
  },
  {
    ...
    description: 'Static content served by raworc-content service, mounted under /content.',
    endpoints: [
      { method: 'GET', path: '/content/health', auth: 'public', desc: 'Health endpoint for the content server.', params: [], example: `curl -s ${BASE}/content/health`, resp: { schema: 'Empty' }, responses: [ { status: 200, body: `{"status":"healthy","service":"raworc-content"}` } ] },
