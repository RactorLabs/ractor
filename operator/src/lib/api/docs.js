...
export function getApiDocs(base) {
  ...
  },
  {
    ...
    endpoints: [
      { method: 'NOTE', path: 'Harmony Composite Format', auth: 'public', desc: 'Agents may post a single composite row per model turn. The field content_json.harmony.segments carries commentary, tool calls/results, and final text in order.', params: [], example: `{
  "role": "agent",
  "channel": "final",
  "content": "All set.",
  "content_json": {
    "harmony": {
      "request_id": "a1b2c3:1",
      "segments": [
        {"type": "commentary", "channel": "analysis", "text": "thinking..."},
        {"type": "tool_call", "tool": "bash", "args": {"command": ["echo", "hi"]}},
        {"type": "tool_result", "tool": "bash", "args": {"command": ["echo", "hi"]}, "output": "hi\n"},
        {"type": "final", "channel": "final", "text": "All set."}
      ]
    }
  }
}` },
      ...
      { method: 'POST', path: '/api/v0/agents/{name}/messages', auth: 'bearer', desc: 'Create a message for agent. User posts are plain text; agents may include Harmony composite fields.', params: [
        ...
        { in: 'body', name: 'content_json', type: 'object|null', required: false, desc: 'Optional structured content. For agents, include { harmony: { request_id, segments: [...] } }.' }
