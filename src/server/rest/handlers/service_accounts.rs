...
use crate::shared::rbac::ServiceAccount;
...
pub async fn delete_service_account(
    ...
    // Service accounts use name as primary key now
    let deleted = state.delete_service_account(&id).await?;
...
pub async fn update_service_account_password(
    ...
    let is_self = match &auth.principal {
        crate::shared::rbac::AuthPrincipal::ServiceAccount(sa) => sa.user == id || sa.id.map(|uuid| uuid.to_string()) == Some(id.clone()),
        _ => false,
    };
    ...
    let updated = state.update_service_account_password(&account.user, &new_pass_hash).await?;
