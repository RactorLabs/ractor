...
pub enum TextEditAction {
    View { path: String, start_line: Option<usize>, end_line: Option<usize> },
    Create { path: String, content: String },
    StrReplace { path: String, target: String, replacement: String },
    Insert { path: String, line: usize, content: String },
}
...
fn normalize_path(p: &str) -> Result<PathBuf> {
    ...
    for c in rel.components() {
        match c {
            ...
            Component::RootDir | Component::Prefix(_) => anyhow::bail!("Absolute paths not allowed"),
        }
    }
...
pub async fn text_edit(action: TextEditAction) -> Result<String> {
    let start_time = std::time::SystemTime::now();
    match action {
        TextEditAction::View { path, start_line, end_line } => {
            ...
            if full.is_dir() {
                ...
                names.sort();
                let out = names.join("\n");
                let _ = save_text_editor_log("view", &path, true, serde_json::json!({"start_line":start_line, "end_line":end_line}), &out, start_time).await;
            ...
            if let (Some(s), Some(e)) = (start_line, end_line) {
                ...
                let out = slice.join("\n");
                let _ = save_text_editor_log("view", &path, true, serde_json::json!({"start_line":s, "end_line":e}), &out, start_time).await;
                info!(tool = "text_edit", action = "view", %path, range = format!("{}-{}", s, e), bytes = out.len(), "tool end");
                Ok(out)
            } else {
                let out = truncate(content);
                let _ = save_text_editor_log("view", &path, true, serde_json::json!({}), &out, start_time).await;
        ...
        TextEditAction::Create { path, content } => {
            ...
            let full = normalize_path(&path)?;
            if let Some(parent) = full.parent() { fs::create_dir_all(parent).await?; }
            let mut f = fs::File::create(&full).await?;
            f.write_all(content.as_bytes()).await?;
            let msg = format!("created: {} ({} bytes)", full.display(), content.len());
            let _ = save_text_editor_log("create", &path, true, serde_json::json!({"length":content.len()}), &msg, start_time).await;
            info!(tool = "text_edit", action = "create", %path, bytes = content.len(), "tool end");
            Ok(msg)
        }
        TextEditAction::StrReplace { path, target, replacement } => {
            ...
            if count != 1 {
                ...
                let _ = save_text_editor_log("str_replace", &path, false, serde_json::json!({"target":target, "replacement":replacement, "count":count}), &err, start_time).await;
            ...
            let _ = save_text_editor_log("str_replace", &path, true, serde_json::json!({"target":target}), &msg, start_time).await;
            info!(tool = "text_edit", action = "str_replace", %path, matches = 1, success = true, "tool end");
            Ok(msg)
        }
        TextEditAction::Insert { path, line, content } => {
            ...
            for (i, l) in lines.iter().enumerate() {
                if i == idx { new_lines.push(content.clone()); }
                new_lines.push((*l).to_string());
            }
            if idx == lines.len() { new_lines.push(content); }
            ...
            let _ = save_text_editor_log("insert", &path, true, serde_json::json!({"line":line}), &msg, start_time).await;
...
async fn save_bash_log(cmd: &str, out: &std::process::Output, start: std::time::SystemTime) -> Result<()> {
    use std::time::UNIX_EPOCH;
    let ts = start.duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0);
    ...
    if let Err(e) = fs::write(&path, content).await { let _ = e; }
    Ok(())
}

async fn save_text_editor_log(command: &str, path_rel: &str, success: bool, params: serde_json::Value, result_msg: &str, start: std::time::SystemTime) -> Result<()> {
    use std::time::UNIX_EPOCH;
    let ts = start.duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0);
