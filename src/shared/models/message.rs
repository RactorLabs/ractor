...
impl AgentMessage {
    pub async fn find_by_id(
        ...
        sqlx::query_as::<_, AgentMessage>(
            r#"SELECT id, agent_name, created_by, role, content, content_json, metadata, created_at
    ...
    pub async fn update_by_id(
        ...
        if let Some(cj) = req.content_json { msg.content_json = Some(cj); }

        sqlx::query(
            r#"UPDATE agent_messages SET content=?, content_json=?, metadata=? WHERE id = ?"#,
        )
        .bind(&msg.content)
    ...
    pub async fn create(
        ...
        sqlx::query(
            ...
            INSERT INTO agent_messages (
                id, agent_name, created_by, role, content, content_json, metadata, created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            "#
        )
    ...
    pub async fn find_by_agent(
        ...
        sqlx::query_as::<_, AgentMessage>(
            r#"
            SELECT id, agent_name, created_by, role,
                   content, content_json, metadata, created_at
            FROM agent_messages
            WHERE agent_name = ?
