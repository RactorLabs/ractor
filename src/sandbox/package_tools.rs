...
impl Tool for PythonPackageTool {
    ...
    async fn execute(&self, args: &serde_json::Value) -> Result<serde_json::Value> {
        ...
        if packages.is_none() {
            return Ok(
                serde_json::json!({"status":"error","tool":"python_package","error":"No packages specified"}),
            );
        }
        ...
        if packages.is_empty() {
            return Ok(
                serde_json::json!({"status":"error","tool":"python_package","error":"No valid packages specified"}),
            );
        }
        ...
        match action {
            ...
            _ => Ok(
                serde_json::json!({"status":"error","tool":"python_package","error":"Invalid action"}),
            ),
        }
    }
}
...
async fn check_packages(packages: &[String]) -> Result<serde_json::Value> {
    ...
    for package in packages {
        let cmd = format!(
            "python -c \"import {}\" 2>/dev/null && echo \"{}:available\" || echo \"{}:missing\"",
    ...
    Ok(
        serde_json::json!({"status":"ok","tool":"python_package","action":"check","output": results.join("\n")}),
    )
}
...
async fn install_packages(packages: &[String], upgrade: bool) -> Result<serde_json::Value> {
    ...
    match run_bash(&cmd).await {
        Ok(output) => Ok(
            serde_json::json!({"status":"ok","tool":"python_package","action":"install","output": output}),
        ),
        Err(e) => Ok(
            serde_json::json!({"status":"error","tool":"python_package","action":"install","error": e.to_string()}),
        ),
    }
}

async fn check_and_install_packages(
    packages: &[String],
    upgrade: bool,
    ...
    Ok(
        serde_json::json!({"status":"ok","tool":"python_package","action":"check_and_install","output": result.join("\n")}),
    )
}
